...
Worker.prototype.handle = function ( data , cb ) {
	...
	var self = this ;
	var pubSub = self._pubSubTransport ;
	...
	pushTransport.receipts.once(errEvt,function(error, errorData){
		...
		switch ( error.toString() ) {
			...
			case 'NotRegistered' :
			case 'TokenUnparseable' :
				pubSub.publish( self._queueNameUnregistered , _.pick( data , 'token' , 'registration_id' ) ) ;
				break;
			case 'CanonicalIdError' :
				pubSub.publish( self._queueNameUnregistered , errorData ) ;
				break;
			case 'PayloadInvalid' :
				self._logger.debug('Invalid payload in message:\n%s\n',JSON.stringify(data)) ;
				break;
			case 'TransmissionError' : 
				self._logger.debug('ReQueueing Message due to TransmissionError:\n%s\n',JSON.stringify(data)) ;
				pubSub.publish( self._queueName , data ) ; // <- requeue
				break ;
			default :
				self._logger.debug('Error processing message:\n%s\n%s\n',error,JSON.stringify(data)) ;
				break ;
		}
	});
