...
var GCMConnection = require('node-gcm');
...
function GCM ( config ) {
	...
	sender = new GCMConnection.Sender(config.apiKey);
};
...
GCM.prototype.send = function ( id, data ) {
	...
	var message = new GCMConnection.Message();

	for(var key in data) {
		message.addData(key, data[key]);
	}

	var regTokens = [ data.registration_id ];

	sender.send(message, {registrationTokens: regTokens}, function(error, response) {
		if(error) {
			console.error(error);
			var parsedError = parseErrorResponse( data ) ;
			if ( ['InternalServerError','QuotaExceeded', 'DeviceQuotaExceeded', 'InvalidServerResponse'].indexOf(parsedError) >= 0 ) {
